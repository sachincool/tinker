...
*Fig. 1 · Two log stores, same hardware, one clear winner on the wall clock.*
...
The Grafana panels behind those numbers use the same six metrics for both systems, two very different shapes:
...
The negative query is the quiet one. At 300 GB Loki handles it in 2.6 seconds. At 500 GB the resources choke and the query halts and never returns. In production that's the difference between an alert that fires and a dashboard that loads.
